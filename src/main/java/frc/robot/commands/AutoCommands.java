...
public class AutoCommands {
    ...
    private Command genericAuto(
        ...
        var previousTarget = ReefPoses.get(initialScoreStep.position);
        for (var autoStep: otherSteps) {
            ...
            if (previousScoreStep.shouldAlign) {
                ...
                previousTraj.atTranslation(goal.getTranslation(), 0.3).onTrue(
                    drivetrain.pathfindCmd(() -> CurrAlliance.red() ? flip(goal) : goal)
                        .withTimeout(3)
            ...
            previousTarget = ReefPoses.get(previousScoreStep.position);
        }
