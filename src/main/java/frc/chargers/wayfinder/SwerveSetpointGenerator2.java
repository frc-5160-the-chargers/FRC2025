...
public class SwerveSetpointGenerator2 {
	...
	private final ArrayList<Optional<Rotation2d>> overrideSteering;
	private final MutTranslation2d chassisForceVec = new MutTranslation2d();
	private final MutTranslation2d moduleForceVec = new MutTranslation2d();
	private final MutTranslation2d chassisAccelVec = new MutTranslation2d();
	private final MutTranslation2d vel = new MutTranslation2d();
	 ...
	 *     per second
	 */
	public SwerveSetpointGenerator2(RobotConfig config, double maxSteerVelocityRadsPerSec) {
		this.config = config;
		this.maxSteerVelocityRadsPerSec = maxSteerVelocityRadsPerSec;
		this.brownoutVoltage = RobotController.getBrownoutVoltage();
		prev_vx = new double[config.numModules];
		prev_vy = new double[config.numModules];
		prev_heading = new Rotation2d[config.numModules];
		desired_vx = new double[config.numModules];
		desired_vy = new double[config.numModules];
		overrideSteering = new ArrayList<>();
		desired_heading = new Rotation2d[config.numModules];
	}
	 ...
	 */
	public SwerveSetpointGenerator2(RobotConfig config, AngularVelocity maxSteerVelocity) {
	...
	public SwerveSetpoint generateSetpoint(
		...
		if (constraints != null) {
			vel.set(desiredStateRobotRelative.vxMetersPerSecond, desiredStateRobotRelative.vyMetersPerSecond);
			double linearVel = vel.getNorm();
			if (linearVel > constraints.maxVelocityMPS()) {
				vel.timesMut(constraints.maxVelocityMPS() / linearVel);
			}
		...
		if (epsilonEquals(desiredStateRobotRelative, ZERO_SPEEDS)) {
			need_to_steer = false;
		...
		for (int m = 0; m < config.numModules; m++) {
			...
			moduleForceVec.set(forceAtCarpet * forceSign * forceAngle.getCos(), forceAtCarpet * forceSign * forceAngle.getSin());
			...
			if (!epsilonEquals(0, moduleForceVec.getNorm())) {
				...
				double thetaRad = moduleForceVec.getAngle().getRadians() - angleToModule.getRadians();
				chassisTorque += forceAtCarpet * config.modulePivotDistance[m] * Math.sin(thetaRad);
			}
		}

		chassisAccelVec.set(chassisForceVec);
		chassisAccelVec.divMut(config.massKG);
		...
		if (constraints != null) {
			...
			if (linearAccel > constraints.maxAccelerationMPSSq()) {
				chassisAccelVec.timesMut(constraints.maxAccelerationMPSSq() / linearAccel);
			}
			chassisAngularAccel =
				MathUtil.clamp(
					chassisAngularAccel,
		...
		return new SwerveSetpoint(retSpeeds, retStates, DriveFeedforwards.zeros(config.numModules));
	}
