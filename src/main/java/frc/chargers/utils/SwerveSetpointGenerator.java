...
import static frc.chargers.utils.UtilMethods.*;
...
public class SwerveSetpointGenerator {
	...
	public record SwerveSetpoint(ChassisSpeeds chassisSpeeds, SwerveModuleState[] moduleStates) {
		public SwerveSetpoint() {
			this(new ChassisSpeeds(), new SwerveModuleState[]{
	...
	private double findRoot(
		...
		double f_1,
		int iterations_left) {
		if (iterations_left < 0 || equivalent(f_0, f_1)) {
			return 1.0;
		}
	...
	private static boolean epsilonEquals(Twist2d receiver, Twist2d other) {
		return equivalent(receiver.dx, other.dx) &&
		       equivalent(receiver.dy, other.dy) &&
		       equivalent(receiver.dtheta, other.dtheta);
	}
	...
	public SwerveSetpoint generateSetpoint(
		final LinearVelocity maxDriveVelocity,
		final LinearAcceleration maxDriveAccel,
		final AngularVelocity maxSteerVelocity,
		...
		if (maxDriveVelocity.in(MetersPerSecond) > 0.0) {
			SwerveDriveKinematics.desaturateWheelSpeeds(desiredModuleState, maxDriveVelocity.in(MetersPerSecond));
		...
		if (epsilonEquals(desiredState.toTwist2d(0.02), new Twist2d())) {
			need_to_steer = false;
		...
		if (all_modules_should_flip
			    && !epsilonEquals(prevSetpoint.chassisSpeeds().toTwist2d(0.02), new Twist2d())
			    && !epsilonEquals(desiredState.toTwist2d(0.02), new Twist2d())) {
			// It will (likely) be faster to stop the robot, rotate the modules in place to the complement
			// of the desired
			// angle, and accelerate again.
			return generateSetpoint(maxDriveVelocity, maxDriveAccel, maxSteerVelocity, prevSetpoint, new ChassisSpeeds(), dt);
		}
		...
		final double max_theta_step = dt * maxSteerVelocity.in(RadiansPerSecond);
		for (int i = 0; i < modules.length; ++i) {
			if (!need_to_steer) {
			...
			if (equivalent(prevSetpoint.moduleStates()[i].speedMetersPerSecond, 0.0)) {
				...
				if (equivalent(desiredModuleState[i].speedMetersPerSecond, 0.0)) {
		...
		final double max_vel_step = dt * maxDriveAccel.in(MetersPerSecondPerSecond);
		for (int i = 0; i < modules.length; ++i) {
			if (min_s == 0.0) {
				// No need to carry on.
				break;
