	...
	public SwerveSetpoint generateSetpoint(
		...
		for (int m = 0; m < config.numModules; m++) {
			...
			if (epsilonEquals(prevSpeed, 0.0)
				...
				if (Math.abs(moduleTorque - config.moduleConfig.torqueLoss) < 1e-9){
					System.out.println("SDKFJKDSKFSLJFLJSDKFJL");
				} else {
					System.out.println(Math.abs(moduleTorque - config.moduleConfig.torqueLoss));
				}
				DogLog.log("ModuleTorqueX(Before)", moduleTorque);
				DogLog.log("TorqueLoss", config.moduleConfig.torqueLoss);
				moduleTorque -= config.moduleConfig.torqueLoss;
				DogLog.log("ModuleTorqueX(After)", moduleTorque);
			...
			DogLog.log("ModuleTorque(before)", moduleTorque);
			DogLog.log("MaxTorqueFriction", config.maxTorqueFriction);
			moduleTorque = Math.min(moduleTorque, config.maxTorqueFriction);
			DogLog.log("ModuleTorque(after)", moduleTorque);
		...
		for (int m = 0; m < config.numModules; m++) {
			...
			double maxVelStep = Math.abs(FeetPerSecond.of(75).in(MetersPerSecond) * dt);
